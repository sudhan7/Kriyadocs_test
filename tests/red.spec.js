...
test('To verify whether the partially validated references have untagged and shown with the red background color', async({browser}) => {
  ...
  //await expect(newPage.locator('.pre-loader-message')).toHaveText('Identifying Author and Affiliation ...');

  const untaggedElements = await newPage.locator(`.back .jrnlRefText[data-untag="true"]`).elementHandles();
  for (const element of untaggedElements)
    {
    const backgroundColor = await newPage.evaluate(el => getComputedStyle(el).backgroundColor, element);
    expect(backgroundColor).toBe('rgb(255, 235, 235)');
    }
