...
test('To verify whether the validated references are tagged and shown with the green background color', async({browser}) => {
  ...
  const validatedElements = await newPage.locator(`[data-validated="true"]`).elementHandles();
  for (const element of validatedElements) {
    ...
    expect(backgroundColor).toBe('rgb(236, 255, 230)');
  }
